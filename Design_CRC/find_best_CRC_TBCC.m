function [d_min,A_min,best_CRC]=find_best_CRC_TBCC(v,gen_CC,K,m,d_max)
...
[TB_messages,d_min,WE]=extract_TB_paths(v,gen_CC,(K+m)/size(gen_CC,1),d_max);
A_min = WE(d_min);
...
% our CRC code
temp_survivors = single(TB_messages);
...
positions_WE=cumsum(WE);
for w=d_min:d_max
    detected_elements = not_surv((not_surv>positions_WE(w-1)) & (not_surv<=positions_WE(w)));
    if numel(detected_elements)>0
        WE(w) = WE(w) - numel(detected_elements);
    end
end
d_min=find(WE,1,'first');
surv=find(burst_len > m);
TB_messages=TB_messages(surv,:);
num_messages = size(TB_messages,1);
length_message = size(TB_messages,2);

% weight matrix (1 in position (i,j) if mex i has trellis paths weight j, 0 otherwise)
...
if up_to_d < d_max
    ...
    mex_1=TB_messages(1:cum_WE(up_to_d),:);
...
while numel(survivors)>0
    ...
    % use the messages of the paths with distance = actual_d
    mex_1=TB_messages(cum_WE(actual_d-1)+1:cum_WE(actual_d),:);
    first_WE = WE(actual_d);
    % check which messages have even numbers of 1s
    indexes_even_mex=find(mod(sum(mex_1,2),2)==0);
    mex_even=mex_1(indexes_even_mex,:);
    first_WE_even=numel(indexes_even_mex);
    ...
    if numel(mex_1)==0
        ...
        for i_CRC = survived
            ...
            if first_WE_even==0 && sum(rem)==0
                ...
                if sum(rem)>0
                    syndromes = (sum(mod(mex_1*H_CRC,2),2)>0).';
                    WE_actual_d = first_WE - sum(syndromes);
                else
                    syndromes = (sum(mod(mex_even*H_CRC,2),2)>0).';
                    WE_actual_d = first_WE_even - sum(syndromes);
                end
...
%% Print results
best_CRC = dec2base(bin2dec(num2str(fliplr(str2num(dec2bin(best_CRC,m+1).').'))),8);
fprintf('\nConcatenation with the best CRC code with degree m=%d has\nd_min=%d, A_min=%d and g_CRC(X)=%s',m,d_min,A_min,best_CRC)
